...
function addPermission(uri, verb, permission) {
  ...
  if (_!permissions[uri]) {
    _permissions[uri] = {};
  }
  _permissions[uri][verb.toUpperCase()] = permission;
}

function splitPermissionUris() {
...
function lookupPermissions(uri, verb) {
  ...
  var uriSegments = uri.replace(/^\/|\/$/g, '').split('/'),
    matchedPermissions = [],
    splitPermissionUris = splitPermissionUris();
...
function generateSimplePermissions() {
  ...
  __.sortBy(Object.keys(_permissions), 'length').forEach(function(permissionUri) {
    var p = _permissions[permissionUri];
