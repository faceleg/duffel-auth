...
function addPermission(uri, verb, permission) {
  ...
  if (_permissions[uri][verb.toLowerCase()]) {
    throw new (error('PermissionCollissionError'))(verb.toUpperCase() + ' ' + uri + ' has the permission: "' +
                                                 _permissions[uri][verb.toLowerCase()] + '" set already. A URI may have a maximum of one permission.');
  }
...
function lookupPermissions(uri, verb) {
  ...
  if (typeof uri !== 'string') {
    throw new error('InvalidArgumentError')('uri is not a string');
  }

  if (typeof verb !== 'string') {
    throw new error('InvalidArgumentError')('verb is not a string');
  }
...
function permissionNames() {
  var permissionNamesList = [];

  flatPermissionTree().forEach(function(permission) {
    Object.keys(permission.permissions).forEach(function(name) {
      if (permissionNamesList.indexOf(permission.permissions[name]) == -1) {
        permissionNamesList.push(permission.permissions[name]);
      }
    });
  });
